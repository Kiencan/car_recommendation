...
# Build content vector based on attributesx
...
def recommend_products_for_profile(profile_id, top_n=8, except_product_ids=None, in_journey_maps=None):
    try:
        # Chuyển đổi các tham số đầu vào thành danh sách nếu cần
        if except_product_ids is None:
            except_product_ids = []
        if in_journey_maps is None:
            in_journey_maps = []
        ...
        # Kiểm tra xem profile có tồn tại trong Qdrant
        if not profile_data or len(profile_data) == 0:
            print(f"Profile {profile_id} not found in Qdrant.")
            return []
        ...
        if not profile.payload:
            print(f"Profile {profile_id} does not have a payload in Qdrant.")
            return []

        # Xây dựng vector profile
        profile_vector = build_profile_vector(
            profile.payload.get('page_view_keywords', []),
            profile.payload.get('purchase_keywords', []),
            profile.payload.get('interest_keywords', []),
            profile.payload.get('journey_maps', [])
        )

        # Nếu không tạo được vector, trả về lỗi
        if profile_vector is None:
            print(f"Error: Could not generate a valid vector for profile {profile_id}.")
            return []

        # Vector tìm kiếm
        search_vector = np.concatenate([profile_vector, profile_vector, profile_vector])
        ...
        # Tìm kiếm trong bộ sưu tập sản phẩm
        search_results = qdrant_client.search(
            collection_name=PRODUCT_COLLECTION,
            query_vector=search_vector,
        ...
        recommended_products = [
            {
                ...
                "product_name": result.payload.get('name'),
                "product_category": result.payload.get('category'),
                "brand": result.payload.get('additional_info', {}).get('brand'),
